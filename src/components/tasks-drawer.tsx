    ...
    return (
        <>
            <button onClick={() => setIsOpen(true)} className="relative p-2 rounded-xl hover:bg-muted transition-colors text-muted-foreground flex items-center justify-center">
                ...
                {pendingCount > 0 && <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-[#00A868] ring-2 ring-card" />}
            </button>
            ...
            <div className={`fixed top-0 right-0 h-screen w-full sm:w-[400px] bg-card border-l border-border z-[101] shadow-2xl flex flex-col transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
                <div className="h-16 flex items-center justify-between px-5 border-b border-border shrink-0 bg-secondary/30">
                    <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#00A868]/20 to-teal-500/20 text-[#00A868] flex items-center justify-center">
                ...
                <div className="p-4 border-b border-border shrink-0">
                    <form onSubmit={addTask} className="relative">
                        <input type="text" placeholder="Adicionar nova tarefa..." value={newTask} onChange={(e) => setNewTask(e.target.value)}
                            className="w-full pl-4 pr-10 py-3 rounded-xl bg-secondary border border-border text-sm focus:ring-2 focus:ring-[#00A868] text-foreground" />
                        <button type="submit" disabled={!newTask.trim()}
                            className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-lg text-[#00A868] hover:bg-[#00A868]/10 disabled:opacity-50 transition-colors">
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-1">
                    {loading ? (
                        <div className="h-full flex items-center justify-center"><Loader2 className="w-6 h-6 animate-spin text-[#00A868]" /></div>
                    ) : allTasks.length === 0 ? (
                            ...
                            .map(task => (
                                <div key={task.id} className={`group flex items-center justify-between gap-2 p-3 rounded-xl transition-all ${task.completed ? "opacity-60" : "bg-secondary hover:bg-secondary/80"}`}>
                                    <div className="flex items-start gap-3 flex-1 min-w-0">
                                        <button onClick={() => toggleTask(task.id, task.completed)} className="shrink-0 mt-0.5 text-muted-foreground hover:text-[#00A868]">
                                            {task.completed ? <CheckCircle2 className="w-5 h-5 text-[#00A868]" /> : <Circle className="w-5 h-5" />}
                                        </button>
                ...
                <div className="p-3 border-t border-border shrink-0">
                    <Link href="/dashboard/tarefas" onClick={() => setIsOpen(false)}
                        className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-[#00A868] hover:bg-[#00A868] text-white rounded-xl text-sm font-medium transition-colors">
