...
body{font-family:Arial,sans-serif;font-size:14px;color:#333;padding:10px}
...
.card h3{font-size:13px;font-weight:bold;text-transform:uppercase;color:#00a868;margin-bottom:3px;letter-spacing:0.3px}
...
.cet-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:0}
...
th{padding:3px 5px;font-weight:600;color:#555;font-size:11px;border-bottom:1px solid #ddd}
...
.footer{font-size:10px;color:#aaa;margin-top:4px;border-top:1px solid #eee;padding-top:2px;display:flex;justify-content:space-between}
</style></head><body>`;
        ...
        html += `<div class="card-row"><span class="label">Tipo</span><span class="val">${ravLabel}</span></div>`;
        ...
        if (maqAdesao > 0) {
            html += `<div class="card-row"><span class="label">Ades\u00e3o (sem aluguel)</span><span class="val" style="color:#1565c0">${maqAdesao} m\u00e1q. (R$ ${adesaoCustoPdf.toFixed(2)})</span></div>`;
        }
        ...
        // RIGHT — CET tables in 2-column grid
    ...
    // Rate Input
    function RI({ l, v, set }: { l: string; v: number; set: (n: number) => void }) {
