        ...
        <div className="h-full flex flex-col">
            ...
            {view === "board" ? (
                ...
                <div className="flex-1 overflow-x-auto -mx-4 lg:-mx-6 px-4 lg:px-6">
                    <div className="flex gap-3 min-h-[calc(100vh-220px)] pb-4" style={{ minWidth: `${STAGES.length * 260}px` }}>
                        {STAGES.map(stage => {
                            const stageNegs = filtered.filter(n => normalizeStatus(n.status) === stage.id);
                            const isDragOver = dragOverStage === stage.id;
                            return (
                                <div key={stage.id}
                                    className={`w-60 shrink-0 flex flex-col rounded-2xl transition-all ${isDragOver ? "ring-2 ring-blue-500/50 bg-blue-500/5" : "bg-card/50"}`}
                                    ...
                                    <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-2">
                                        {stageNegs.map(neg => (
                                            <div key={neg.id} draggable
                                                ...
                                                className={`bg-card border border-border rounded-xl p-3 cursor-grab active:cursor-grabbing hover:shadow-md hover:border-blue-500/30 transition-all group ${dragId === neg.id ? "opacity-50 scale-95" : ""}`}>
                                                <div className="flex items-start justify-between gap-1 mb-1.5">
                                                    <p className="text-sm font-semibold text-foreground leading-tight">{neg.clientName}</p>
                                                    <GripVertical className="w-3.5 h-3.5 text-muted-foreground/30 shrink-0 mt-0.5" />
                                                </div>
                                                {neg.stoneCode && <p className="text-[10px] text-muted-foreground mb-1.5">SC: {neg.stoneCode}</p>}

                                                {/* Mini rates */}
                                                <div className="grid grid-cols-3 gap-1 mb-2">
                                                    {[{ l: "Déb", v: neg.rates.debit }, { l: "1x", v: neg.rates.credit1x }, { l: "PIX", v: neg.rates.pix }].map(r => (
                                                        <div key={r.l} className="bg-secondary/60 rounded-md px-1.5 py-1 text-center">
                                                ...
                                                <div className="flex items-center gap-1 flex-wrap">
                                                    <span className="text-[9px] text-muted-foreground flex items-center gap-0.5 bg-muted/50 px-1.5 py-0.5 rounded">
                                                        <Calendar className="w-2.5 h-2.5" /> {fmtDate(neg.dateNeg)}
                                                    </span>
                                                    {neg.assignee && (
                                                        <span className="text-[9px] text-purple-500 flex items-center gap-0.5 bg-purple-500/10 px-1.5 py-0.5 rounded font-medium">
                                                            <User className="w-2.5 h-2.5" /> {neg.assignee.name.split(" ")[0]}
                                                        </span>
                                                    )}
                                                    {neg.dateAccept && (
                                                        <span className="text-[9px] text-emerald-500 flex items-center gap-0.5 bg-emerald-500/10 px-1.5 py-0.5 rounded font-medium">
                                                            <CheckCircle className="w-2.5 h-2.5" /> {fmtDate(neg.dateAccept)}
                                                        </span>
                                                    )}
                                                </div>
                                                ...
                                                {neg.stageHistory && neg.stageHistory.length > 0 && (
                                                    <div className="mt-1.5 pt-1.5 border-t border-border/30">
                                                        <div className="flex items-center gap-0.5 text-[8px] text-muted-foreground/60">
