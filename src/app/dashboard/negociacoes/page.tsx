...
import {
    Handshake, Plus, X, LayoutGrid, List, Search,
    Calendar, CalendarPlus, CalendarDays, MessageSquare, Clock, User, Trash2, CheckCircle,
    AlertCircle, Loader2, GripVertical, ArrowRight, FileDown
} from "lucide-react";
import {
    StageBadge, NegotiationDrawer, NewClientForm,
    Client, UserOption, RateSnapshot, Negotiation,
    STAGES, normalizeStatus, getStage, getLeadAge, getTransitionPrompt, TRANSITION_PROMPTS,
    initials, fmtDate, fmtDateTime, today, defaultRates, gcalLink, shareWhatsApp
} from "./components";
...
export default function NegociacoesPage() {
    ...
    const [view, setView] = useState<"board" | "list" | "new">("board");
    ...
    // New client save
    async function handleSaveClient(data: { name: string; stoneCode: string; cnpj: string; phone: string; email: string; rates: RateSnapshot; dateNeg: string; notes: string; assigneeId?: string }) {
        try {
            const r = await fetch("/api/clients", {
                method: "POST", headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    name: data.name, stoneCode: data.stoneCode, cnpj: data.cnpj, phone: data.phone, email: data.email,
                    negotiation: { dateNeg: data.dateNeg, rates: data.rates, notes: data.notes, assigneeId: data.assigneeId, status: "prospeccao" },
                }),
            });
            if (r.ok) { setMsg({ type: "ok", text: `${data.name} adicionado ao pipeline!` }); setView("board"); loadAll(); }
    ...
    /* ═══ NEW CLIENT FORM ═══ */
    if (view === "new") {
        return <NewClientForm users={users} onSave={handleSaveClient} onCancel={() => setView("board")} />;
    }
        ...
        <div className="h-full flex flex-col">
            ...
            <div className="flex items-center justify-between pb-4 gap-3 flex-wrap">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    <div className="flex items-center gap-0.5 bg-muted/40 p-0.5 rounded-lg overflow-x-auto">
                        {(["today", "week", "month", "all"] as const).map(tf => {
                            ...
                            return (
                                <button key={tf} onClick={() => setTimeFilter(tf)}
                                    className={`px-2.5 py-1.5 rounded-md text-xs font-medium transition-all whitespace-nowrap ${timeFilter === tf ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>
                                    {labels[tf]}
                                </button>
                            );
                        })}
                            ...
                            return (
                                <div key={stage.id}
                                    ...
                                    <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-2">
                                        ...
                                        {visible.map(neg => {
                                            ...
                                            <div key={neg.id} draggable
                                                ...
                                                <div className="flex items-center gap-1 mt-2 pt-2 border-t border-border/40">
                                                    <button onClick={(e) => { e.stopPropagation(); shareWhatsApp(neg); }} className="flex-1 py-1.5 rounded-lg text-[10px] font-semibold text-[#00A868] bg-[#00A868]/8 hover:bg-[#00A868]/15 flex items-center justify-center gap-0.5 transition-colors" title="WhatsApp">
                                                        <MessageSquare className="w-3 h-3" /> Zap
                                                    </button>
                                                    <a href={gcalLink(neg)} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="flex-1 py-1.5 rounded-lg text-[10px] font-semibold text-blue-500 bg-blue-500/8 hover:bg-blue-500/15 flex items-center justify-center gap-0.5 transition-colors" title="Calendário">
                                                        <CalendarPlus className="w-3 h-3" /> Agendar
                                                    </a>
                                                    <button onClick={(e) => { e.stopPropagation(); generateProposalPDF(
                                                    ...
                                                    <button onClick={(e) => { e.stopPropagation(); deleteNeg(neg.id); }} className="py-1.5 px-2 rounded-lg text-[10px] font-semibold text-red-400/60 hover:text-red-500 hover:bg-red-500/10 transition-colors" title="Excluir">
                                        ...
                                        {stageNegs.length === 0 && (
                                            <div className="flex flex-col items-center justify-center py-8 text-muted-foreground/30"><Handshake className="w-8 h-8 mb-1" /><p className="text-[10px]">Nenhuma</p></div>
                                        )}
                                    </div>
                                </div>
                            );
                ...
                <div className="flex-1 overflow-auto">
                    <div className="space-y-2">
                        {filtered.length === 0 ? (
                            ...
                            <div key={neg.id} onClick={() => setDrawerNeg(neg)} className="card-elevated rounded-xl p-4 flex items-center gap-4 hover:border-[#00A868]/30 transition-all cursor-pointer">
                                ...
                                <div className="flex items-center gap-2 shrink-0">
                                    {neg.assignee && <span className="text-[10px] bg-purple-500/10 text-purple-500 px-2 py-0.5 rounded-full font-medium hidden sm:inline">{neg.assignee.name.split(" ")[0]}</span>}
                                    ...
                                    <div className="flex gap-0.5">
                                        ...
                                        <button onClick={(e) => { e.stopPropagation(); shareWhatsApp(neg); }} className="p-1.5 rounded-lg bg-[#00A868]/10 text-[#00A868] hover:bg-[#00A868]/20" title="WhatsApp"><MessageSquare className="w-3.5 h-3.5" /></button>
                                        <button onClick={(e) => { e.stopPropagation(); deleteNeg(neg.id); }} className="p-1.5 rounded-lg bg-red-500/10 text-red-500 hover:bg-red-500/20"><Trash2 className="w-3.5 h-3.5" /></button>
                                    </div>
                                </div>
                            </div>
                        ))}
